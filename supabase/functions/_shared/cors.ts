...
export function getCorsHeaders(req: Request): Record<string, string> {
  ...
  const isVercelPreview = origin.endsWith('-ruiz-inmobiliarias-projects.vercel.app')
  const allowedOrigin = ALLOWED_ORIGINS.includes(origin) || isVercelPreview ? origin : ALLOWED_ORIGINS[0]
  return {
